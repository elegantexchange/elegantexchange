...
function splitWords(consignorPct = 50) {
  const c = Math.round(Number(consignorPct) * 100) / 100;
  const s = Math.round((100 - c) * 100) / 100;
  if (c === 50 && s === 50) return "fifty / fifty (50% / 50%)";
  return `${s}% Store / ${c}% Consignor`;
}
...
export function buildAgreementText({
  ...
  const splitLabel = splitWords(consignorSplitPct);
...
2. COMMISSION SPLIT. The Store and the Consignor agree to a ${splitLabel} split on the final sale price of each item, calculated automatically and itemized in the Store's records.
